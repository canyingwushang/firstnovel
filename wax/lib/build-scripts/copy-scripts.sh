...
if [ $WAX_COMPILE_SCRIPTS ]; then
    ...
    lua "$PROJECT_DIR/wax/lib/build-scripts/luac.lua" "" run.dat "$SOURCE_SCRIPTS_DIR/" "$SOURCE_SCRIPTS_DIR/run.lua" -L "$SOURCE_SCRIPTS_DIR"/**/*.lua
    mv run.dat "$DESTINATION_SCRIPTS_DIR"
